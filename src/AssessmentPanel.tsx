...
export function AssessmentPanel({ subjectId }: { subjectId:string }) {
  const [qs, setQs] = useState<Q[]>([]);
  const [answers, setAnswers] = useState<Record<string,string>>({});
  const [grades, setGrades] = useState<Record<string,{score:number; reasoning:string}>>({});
  ...
  async function gen() {
    ...
    } catch { setQs([{id:"mock1",type:"mcq",prompt:"Mock: What is a limit?",topic:"Limits",citation:"Syllabus p1",rubric:"Correct definition"}]); }
    setLoading(false);
  }
  ...
  async function grade(id:string) {
    ...
    try {
      const fd = new FormData(); fd.append("prompt", q.prompt); fd.append("answer", ans); fd.append("rubric", q.rubric);
      const r = await fetch("http://localhost:1421/assess/grade",{method:"POST", body: fd});
      const j = await r.json(); setGrades(g=>({...g,[id]:{score:j.score,reasoning:j.reasoning}}));
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {qs.map(q=>(
        <div key={q.id} className="file" style={{gap:8}}>
          ...
          {grades[q.id] && <div style={{fontSize:12}}>Score: <b>{grades[q.id].score}%</b> — {grades[q.id].reasoning} <button className="u-btn" onClick={()=>grade(q.id)} style={{marginLeft:6}}>Re-grade (dispute)</button></div>}
        </div>
      ))}
    </div>
  );
