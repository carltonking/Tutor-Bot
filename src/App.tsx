    ...
    <div className="app">
      ...
      <main className="center">
        ...
        <div className="input-bar">
          ...
          <input className="chat-input" placeholder={`Message ${active.name}`} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={async (e)=>{ if(e.key==="Enter" && input.trim()){ const u=input; setInput(""); setChatLog(c=>[...c,{role:"user",text:u}]); try{ const fd=new FormData(); fd.append("subject_id",selected); fd.append("message",u); const r=await fetch(`http://localhost:1421/chat`,{method:"POST", body: fd}); const j=await r.json(); setChatLog(c=>[...c,{role:"bot",text:j.answer + (j.via==="zen" ? "" : " (template — set OPENCODE_ZEN_BASE_URL if needed)")}]);}catch{ try{ const r=await fetch(`http://localhost:1421/search/${selected}?q=${encodeURIComponent(u)}&k=2`); const j=await r.json(); const cite=j[0]?.text?.slice(0,120)||"no sources yet — upload PDFs"; setChatLog(c=>[...c,{role:"bot",text:`(${active.name} — citing: "${cite}...") Here's the grounded answer.`}]);}catch{ setChatLog(c=>[...c,{role:"bot",text:`Noted for ${active.name}. (sidecar offline — mock)`}]); } } } }} />
          <button className="mic-btn" onClick={async ()=>{ if(!input.trim()) return; const u=input; setInput(""); setChatLog(c=>[...c,{role:"user",text:u}]); try{ const fd=new FormData(); fd.append("subject_id",selected); fd.append("message",u); const r=await fetch(`http://localhost:1421/chat`,{method:"POST", body: fd}); const j=await r.json(); setChatLog(c=>[...c,{role:"bot",text:j.answer}]);}catch{ setChatLog(c=>[...c,{role:"bot",text:`Noted.`}]); } }}>➤</button>
        </div>
      </main>
